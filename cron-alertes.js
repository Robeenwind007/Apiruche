...
async function main() {
  ...
  ruches.forEach(r => {
    ...
    if (r.remerage_division && r.date_remerage_division) {
      ...
      if (diff >= 14) {
        ...
        alertes.push({ emoji: 'Reine', nom: r.nom || r.id, action: 'Remerage/Division', label });
      }
    }

    // --- Poser une hausse ---
    const haussesRuche = hausses.filter(h => h.ruche_id === r.id);
    if (haussesRuche.length === 0) {
      // Dernière pesée de cette ruche
      const pRuche = pesees
  ...
  const lignes = alertes.map(a => {
    let txt = `*${normalize(a.nom)}* - ${normalize(a.action)}`;
    if (a.label) txt += `\n   ${normalize(a.label)}`;
    return txt;
  });
